...
How to handle images in any premium website without asking the user to do anything technical. Four sources (this is capability ③, the "image genie"):

1. **User has photos** → run `scripts/webp_convert.py` to convert to WebP automatically.
2. **User wants stock images** → run `scripts/openverse_fetch.py` to download from Openverse (free, CC-licensed).
3. **AI-generated (OpenAI gpt-image-2)** → bespoke, on-brand imagery when stock would look generic, or to clean ugly supplier photos into pro shots. Full method in **`reference/11-ai-image-generation.md`** (opt-in, costs the user a few $). The generated files feed this same pipeline: JPEG → (crop banners) → `webp_convert.py` → `assets/img/*.webp`.
4. **Mix** → any combination, with credits attributed for the Openverse ones.

Whatever the source, the site references images identically and they end as **WebP in `assets/img/`** — never mix formats.
