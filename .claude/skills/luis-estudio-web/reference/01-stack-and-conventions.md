...
- **9. Webs Preparadas para Automatización (Arquitectura Dual / JSON)**: Si en la Fase de Briefing el usuario solicita el módulo de "Blog Automatizado", NO escribas los artículos directamente (hardcoded) en el HTML. Debes programar el frontend para que lea dinámicamente de un archivo `data/posts.json` usando `fetch()`. Esto permite que robots externos (n8n, Make) puedan inyectar contenido simplemente modificando el archivo JSON, creando un CMS completamente automático sin base de datos.
